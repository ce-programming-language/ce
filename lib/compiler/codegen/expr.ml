  ...
  and gen_call env compile_stmt_cb loc name targs args =
    ...
    if Option.is_some builtin_opt then
      ...
      | TFn (param_tys, ret_ty) ->
          ...
          let arg_vals =
            process_args env compile_stmt_cb loc expected_tys param_tys args 1
  ...
  and gen_method_call env compile_stmt_cb loc name args =
    ...
    if Hashtbl.mem env.struct_registry base_path then (
      ...
      let coerced_self =
        ...
        else coerce_value env loc expected_self_ty self_val false false
      in
      let arg_vals =
