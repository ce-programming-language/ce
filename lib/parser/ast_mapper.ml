  ...
  object (self)
    ...
    method map_expr (e : expr) : expr =
      ...
      | Call (name, targs, args) ->
          Call (name, List.map self#map_type targs, List.map self#map_expr args)
      | Array (n, ty, elems) ->
      ...
      | Struct (name, targs, fields) ->
          ...
          Struct (name, List.map self#map_type targs, List.map map_field fields)
    ...
    method map_stmt (s : stmt) : stmt =
      ...
      | DerefAssign (ptr, e) -> DerefAssign (self#map_expr ptr, self#map_expr e)
